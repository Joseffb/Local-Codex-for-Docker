...
use codex_core::config::docker_mcp::DockerMcpSetupChoice;
use codex_core::config::docker_mcp::docker_mcp_setup_edits;
use codex_core::config::docker_mcp::docker_mcp_toolkit_available;
use codex_core::config::docker_mcp::should_prompt_docker_mcp_setup;
...
async fn maybe_prompt_docker_mcp_setup(interactive: &TuiCli) -> std::io::Result<()> {
    if !(std::io::stdin().is_terminal() && std::io::stderr().is_terminal()) {
        return Ok(());
    }

    let cli_overrides = interactive
        .config_overrides
        .parse_overrides()
    ...
    let config = ConfigBuilder::default()
        ...
        .build()
        .await?;
    let effective_servers = config
        .mcp_servers
        .get()
        .iter()
        .map(|(name, server)| (name.clone(), server.clone()))
        .collect::<BTreeMap<_, _>>();
    if !should_prompt_docker_mcp_setup(
        config.docker_mcp_auto_configure,
        &effective_servers,
        docker_mcp_toolkit_available(),
    ) {
        return Ok(());
    }

    let global_servers = codex_core::config::load_global_mcp_servers(&config.codex_home).await?;
    let choice =
        if confirm_default_yes("Docker MCP Toolkit detected. Configure automatically? [Y/n]")? {
            DockerMcpSetupChoice::Accept
        } else {
            DockerMcpSetupChoice::Decline
        };
    let edits = docker_mcp_setup_edits(&global_servers, choice);
    let mut builder = ConfigEditsBuilder::for_config(&config)
        .set_docker_mcp_auto_configure(edits.docker_mcp_auto_configure.unwrap_or(false));
    if let Some(servers) = edits.mcp_servers.as_ref() {
        builder = builder.replace_mcp_servers(servers);
    }
    builder.apply().await.map_err(std::io::Error::other)
}
...
async fn run_interactive_tui(
    ...
    maybe_prompt_docker_mcp_setup(&interactive).await?;
    let start_tui = || {
        codex_tui::run_main(
            interactive.clone(),
            arg0_paths.clone(),
...
fn confirm_default_yes(prompt: &str) -> std::io::Result<bool> {
    eprintln!("{prompt}");
