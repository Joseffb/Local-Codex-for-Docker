...
impl ModelProviderInfo {
    ...
    pub fn create_docker_model_runner_provider() -> ModelProviderInfo {
        create_chat_completions_provider(
            DOCKER_MODEL_RUNNER_PROVIDER_NAME,
            DOCKER_MODEL_RUNNER_DEFAULT_BASE_URL,
        )
    }

    pub fn create_docker_model_gateway_provider() -> ModelProviderInfo {
        create_chat_completions_provider(
            DOCKER_MODEL_GATEWAY_PROVIDER_NAME,
            DOCKER_MODEL_GATEWAY_DEFAULT_BASE_URL,
        )
    }
    ...
    pub fn is_docker_model_runner(&self) -> bool {
        self.name == DOCKER_MODEL_RUNNER_PROVIDER_NAME
            || self
                .base_url
                .as_deref()
                .map(|base_url| base_url.trim_end_matches('/'))
                .is_some_and(|base_url| base_url == DOCKER_MODEL_RUNNER_DEFAULT_BASE_URL)
    }

    pub fn is_docker_model_gateway(&self) -> bool {
        self.name == DOCKER_MODEL_GATEWAY_PROVIDER_NAME
            || self
                .base_url
                .as_deref()
                .map(|base_url| base_url.trim_end_matches('/'))
                .is_some_and(|base_url| base_url == DOCKER_MODEL_GATEWAY_DEFAULT_BASE_URL)
    }

    pub fn uses_docker_model_context_variants(&self) -> bool {
        self.is_docker_model_runner() || self.is_docker_model_gateway()
    }
...
pub fn built_in_model_providers(
    ...
    let docker_model_runner_provider = P::create_docker_model_runner_provider();
    let docker_model_gateway_provider = P::create_docker_model_gateway_provider();
    ...
    [
        ...
        (
            DOCKER_MODEL_RUNNER_PROVIDER_ID,
            docker_model_runner_provider,
        ),
        (
            DOCKER_MODEL_GATEWAY_PROVIDER_ID,
            docker_model_gateway_provider,
        ),
        (
...
fn create_chat_completions_provider(name: &str, base_url: &str) -> ModelProviderInfo {
    ModelProviderInfo {
        name: name.into(),
