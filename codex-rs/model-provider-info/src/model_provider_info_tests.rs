...
fn test_serialize_chat_completions_wire_api() {
    let provider = ModelProviderInfo {
        name: "Docker Model Runner".into(),
        base_url: Some("http://localhost:12434/engines/v1".into()),
...
#[test]
fn test_built_in_model_providers_include_docker_model_runner() {
    let providers = built_in_model_providers(/*openai_base_url*/ None);
    let provider = providers
        .get(DOCKER_MODEL_RUNNER_PROVIDER_ID)
        .expect("Docker Model Runner provider should be built in");

    assert_eq!(
        provider,
        &ModelProviderInfo {
            name: "Docker Model Runner".to_string(),
            base_url: Some(DOCKER_MODEL_RUNNER_DEFAULT_BASE_URL.to_string()),
            env_key: None,
            env_key_instructions: None,
            experimental_bearer_token: None,
            auth: None,
    ...
    assert!(provider.is_docker_model_runner());
}

#[test]
fn test_docker_model_runner_detection_accepts_default_runner_base_url() {
    let provider = ModelProviderInfo {
        name: "Local Docker-compatible endpoint".to_string(),
        base_url: Some(DOCKER_MODEL_RUNNER_DEFAULT_BASE_URL.to_string()),
...
#[test]
fn test_docker_model_context_variant_detection_covers_runner_and_gateway() {
    let providers = built_in_model_providers(/*openai_base_url*/ None);
    ...
    assert!(
        providers
            .get(DOCKER_MODEL_RUNNER_PROVIDER_ID)
            .expect("Docker Model Runner provider should be built in")
            .uses_docker_model_context_variants()
    );
    assert!(
        providers
            .get(DOCKER_MODEL_GATEWAY_PROVIDER_ID)
            .expect("Docker Model Gateway provider should be built in")
            .uses_docker_model_context_variants()
    );

    let arbitrary_chat_provider =
        create_oss_provider_with_base_url("http://localhost:8080/v1", WireApi::ChatCompletions);
    assert!(!arbitrary_chat_provider.uses_docker_model_context_variants());
}

#[test]
fn test_built_in_model_providers_include_docker_model_gateway() {
    let providers = built_in_model_providers(/*openai_base_url*/ None);

    assert_eq!(
        providers.get(DOCKER_MODEL_GATEWAY_PROVIDER_ID),
        Some(&ModelProviderInfo {
            name: "Docker Model Gateway".to_string(),
            base_url: Some(DOCKER_MODEL_GATEWAY_DEFAULT_BASE_URL.to_string()),
            env_key: None,
            env_key_instructions: None,
            experimental_bearer_token: None,
            auth: None,
